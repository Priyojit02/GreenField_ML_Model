    ...
    for col in df.columns:
        ...
        if mapped is None:
            close = get_close_matches(
                key, [c.lower() for c in CANONICAL_COLS], n=1, cutoff=0.8
            )
            if close:
                mapped = CANONICAL_COLS[
                    [c.lower() for c in CANONICAL_COLS].index(close[0])
                ]
        if mapped is None:
...
def evaluate_on_same_split(
    ...
    for name, mdl in models.items():
        ...
        r2 = (
            r2_score(y_te, preds)
            if len(y_te) >= 2 and pd.Series(y_te).nunique() > 1
            else np.nan
        )
...
def main():
    ...
    for target in CANONICAL_COLS:
        ...
        if len(X) < 6:
            print(f"[{target}] ⏭ skipped (too few usable rows: {len(X)}).\n")
            continue
        ...
        best_name = max(
            results.keys(),
            key=lambda k: (
                np.nan_to_num(results[k]["r2"], nan=-1e9),
                -results[k]["mae"],
            ),
        )
    ...
    print("\n=== ✅ Fixed Split Scores (8 train / 5 test) ===")
    if reports:
